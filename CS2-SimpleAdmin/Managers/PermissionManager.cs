		...
		var jsonData = validPlayers
			...
			.ToDictionary(
				...
				object (group) =>
				{
					...
					if (SteamID.TryParse(consolidatedData.identity, out var steamId) && steamId != null)
					{
						AdminCache.TryAdd(steamId, (group.First().ends, consolidatedData.flags));
					}

					// Return consolidatedData directly without queueing a cache clear for every single player!
